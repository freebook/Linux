...
<xsl:template match="*" mode="recursive-chunk-filename">
  ...
  <xsl:variable name="filename">
    <xsl:choose>
      ...
      <!-- Special case -->
      <xsl:when test="self::d:legalnotice and $generate.legalnotice.link != 0">
  ...
  <xsl:if test="$suppress.navigation = '0' and $suppress.header.navigation = '0'">
    <div class="navheader">
      ...
      <xsl:if test="$header.rule != 0">
        <hr/>
		<a href="http://netkiller.8800.org/">Home</a> | <a href="http://neo.abula.com.cn">Mirror1</a> | <a href="http://netkiller.hikz.com/">Mirror2</a>
      </xsl:if>
    </div>
  </xsl:if>
</xsl:template>
